...
function createModel() {
    ...
    model.add(tf.layers.dense({inputShape: [1], units: 20, useBias: true})); // dense layer adds bias by default
...
function testModel(model, inputData, normalizationData) {
    ...
    const [xs, preds] = tf.tidy(() => {
        ...
        const xs = tf.linspace(0, 1, 100); // 生成均匀间隔的范围从0-1的100个数字
        const preds = model.predict(xs.reshape([100, 1])); // 输入数据需转成tensor
        preds.print(); 
